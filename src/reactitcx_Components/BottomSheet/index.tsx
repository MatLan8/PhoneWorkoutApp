...
/** Compose Reanimated's animated ref callback with an optional consumer ref. */
function mergeScrollableRef(
  animatedRef: (instance: HostInstance | null) => void,
  existingRef: React.Ref<HostInstance | null> | undefined,
): React.RefCallback<HostInstance | null> {
  return (node) => {
    animatedRef(node);
    if (typeof existingRef === "function") {
      existingRef(node);
    } else if (existingRef != null) {
      (existingRef as React.MutableRefObject<HostInstance | null>).current =
        node;
    }
  };
}

function elementHostRef(
  element: ReactElement,
): React.Ref<HostInstance | null> | undefined {
  const anyEl = element as ReactElement & {
    ref?: React.Ref<HostInstance | null>;
  };
  return (
    anyEl.ref ?? (element.props as { ref?: React.Ref<HostInstance | null> }).ref
  );
}
...
const BottomSheetComponent = forwardRef<BottomSheetMethods, BottomSheetProps>(
  (
    ...
    /** RNGH / RN FlatList expect a real function; Reanimated's handler is an object. */
    const onScrollForList = useCallback(
    ...
    const scrollPropsForList: Partial<ScrollViewProps> = useMemo(
      () => ({
        scrollEnabled: enableScroll,
        onScroll: onScrollForList,
    ...
    const renderContent = useCallback(() => {
      ...
      if (childArray.length === 1 && isScrollableList(childArray[0])) {
        const listElement = childArray[0] as ReactElement<ScrollViewProps>;
        const enhancedList = cloneElement(
          listElement as ReactElement<Record<string, unknown>>,
          {
            ...scrollPropsForList,
            ref: mergeScrollableRef(
              scrollViewRef as unknown as (
                instance: HostInstance | null,
              ) => void,
              elementHostRef(listElement),
            ),
            onScroll: (event: NativeSyntheticEvent<NativeScrollEvent>) => {
              onScrollForList(event);
              const userOnScroll = listElement.props.onScroll;
      ...
      if (hasScrollableChild) {
        const enhancedChildren = childArray.map((child, index) => {
          if (isScrollableList(child)) {
            const listElement = child as ReactElement<ScrollViewProps>;
            return cloneElement(
              listElement as ReactElement<Record<string, unknown>>,
              {
